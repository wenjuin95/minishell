...
static int	main_dir(t_minishell *m_shell)
{
	...
	update_env("OLDPWD", getenv("PWD"), false, m_shell); //update OLDPWD to PWD
	home = getenv("HOME");
	if (home == NULL)
	{
...
static int	return_dir(t_minishell *m_shell)
{
	...
	if (chdir(getenv("OLDPWD")) == 0) //to chg to OLPWD dir and update PWD to OLDPWD
	{
		update_env("PWD", getenv("OLDPWD"), false, m_shell);
		return (0);
	}
	return (1);
}
...
int	cd_option(t_minishell *m_shell, char **cmd)
{
	...
	update_env("OLDPWD", getenv("PWD"), false, m_shell);
