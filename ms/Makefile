...
#    Updated: 2024/10/10 16:14:41 by tkok-kea         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
# src and objs
SRC_DIR		:=	scanner parser execution expansion builtin utils expansion/wildcard
SRC_DIR		:=	${addprefix srcs/, ${SRC_DIR}}
SRC_DIR		+=	srcs
vpath %.c	${SRC_DIR}

SCANNER		=	scanner.c scanner_utils.c
PARSER		=	parser.c parser_utils.c parser_utils2.c tree_utils.c \
				parse_simple_cmd.c
...
EXPAND		=	expansion.c expansion_steps.c expansion_handler.c \
				...
				ft_str_replace.c \
				expand_wildcard.c is_match.c
EXECUTE		=	execution.c execution_logic_sub.c \
				execution_utils.c execution_utils2.c ft_execvpe.c \
				execution_utils3.c
SRCS		=	minishell.c \
...
fsan: ${OBJS} | ${LIBFT}
			@${CC} ${CFLG} ${FSAN} ${OBJS} ${LIBFT} ${RDL_LIB} -lncurses -o ${NAME}
			@printf "${GREEN}${BOLD}** Compiled ${NAME} with %s **\n${RESET}" $@
...
.phony: all bonus clean fclean re objects norm fsan
